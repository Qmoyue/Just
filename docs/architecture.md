...
| 候选知识源 | 借鉴 | 作用 | 计划 |
|---|---|---|---|
| **KS3 链可行性验证 PASM-lite（已实现）** | Gadget Inspector 的 PASM（Partial Assembly，类型约束装配） | 沿链逐跳验证运行时类型约束；只拒绝可证明不可能的链 | v0.1 ✓ |
| **KS4 前向对象污点引擎（已实现）** | Gadget Inspector 前向两段式（方法摘要 + 事实不动点） | 从 magic entry/OIS 正向传播"反序列化对象"污点，不逐调用点枚举 | v0.1 ✓ |
| **KS5 分配点敏感 + 接口/代理/反射补全（已实现）** | tabby（Soot points-to）；Gadget Inspector 对象图 | 污点对象按创建点（OIS 读/NEW）跟踪路径；污点命中接口调用（实现>枚举上限未物化）时按上限展开实现（this/实参双路）；Proxy.newProxyInstance 的 handler 串入；Method.invoke 常量方法名反射解析；thisTainted 类字段读即污点；类级对象污点向子类型传播 | v0.1 ✓ |
| KS6 反射/代理/lambda 按需解析（可选并入 KS5） | CodeQL models-as-data | 反向遇反射/代理/indy 时向黑板提需求，解析后重新投递（EDGE_ADDED 反馈环） | v0.2 |

**KS3 PASM 校验规则（保守，只拒绝可证明不可能的链）：**

```text
沿链（entry→sink）维护当前值的可能运行时类型集：
1. FIELD_FLOW(f)：字段 f 必须声明于 fromOwner 或其父类，否则拒绝
2. 调用跳：目标方法必须在 toOwner 上可解析（声明或继承），否则拒绝
3. LAMBDA / 无描述符 / 类不可解析：保守通过
```

**KS5 分配点敏感 + 接口/代理补全（前向引擎增强，与 KS4 共用引擎、按选项区分）：**

```text
ForwardEngine(Options{expandInterfaces, threadProxy, reflectiveResolve})
KS4 = 粗扫（全部关闭）：全类级事实，快
KS5 = 精扫（全部开启）：
  1. expandInterfaces：污点 receiver/实参命中接口调用且出边≤1（仅声明目标，
     实现>枚举上限未物化）时，按上限（200）展开实现类 addThis/addParam
     —— 打通 Comparator.compare→FuncComparator、Property.getValue→NestedMethodProperty
  2. threadProxy：receiver 为 Proxy.newProxyInstance 结果时，handler 实参
     → addThis(handler 类) —— 打通 Shape 代理→MapProxy.invoke
  3. reflectiveResolve：Method.invoke 的 Method 对象来自
     getMethod/getDeclaredMethod 且方法名为常量时，解析目标方法 addThis/addParam
     —— 打通 MethodProperty.getValue→getOutputProperties 类反射执行
```
